...
On large products, shipping a new design system components is only half the battle. Getting the components into use across the codebase is a significant challenge. The design system team ships new components, but the legacy parts of the product keep using older patterns. Teams want to update the code, but the work is slow.
...
You don't need smarter model to fix this issues. The real game changer here is the corpus. By feeding the agent your team's actual git history, you're giving it a roadmap of successful migrations instead of making it guess from scratch.
...
Most RAG tutorials index your documentation. That is reasonable for many use cases, but for a design system it is not enough. Documentation tells the agent what components exist and what their props are. But it does not tell the agent how to update legacy code to use them.
...
The actual code for this setup is surprisingly simple. The real effort (and the real payoff) lies in curating the right data.
